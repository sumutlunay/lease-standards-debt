...
BOX_MAIN   = 'https://ucdavis.box.com/shared/static/ojufkahun1muhm1qt9fe40f2pdle7hi3.csv'
BOX_COUNTS = 'https://ucdavis.box.com/shared/static/kai52qfphwdbt1mlhgibvto2lzdv8294.csv'

def load_csv(path_or_url: str) -> pd.DataFrame:
    """Load a CSV from a local path or a Box shared-link URL."""
    if path_or_url.startswith('http'):
        r = requests.get(path_or_url, allow_redirects=True)
        r.raise_for_status()
...
def build_rhs_aa_columns_LOG_IND(df_sub, event_key, sample_key, counts):
    ...
    # Column order matches paper: indicators first, then logs (NonTop5_Unrelated, NonTop5_Related, Top5_Unrelated, Top5_Related)
    rhs = pd.DataFrame({
        'Ipos_AA_Other_outside': ind_cols['Ipos_AA_Other_outside'],
        'Ipos_AA_Other_inside':  ind_cols['Ipos_AA_Other_inside'],
        'Ipos_AA_Top5_outside':  ind_cols['Ipos_AA_Top5_outside'],
        'Ipos_AA_Top5_inside':   ind_cols['Ipos_AA_Top5_inside'],
...
FE_PREFIXES = ('iy_', 'b_', 'l_', 'p_')

PAPER_EVENTS       = ['fr', 'aqrm_mi', 'aec', 'ic_m', 'ic_a', 'aqrm_gc', 'aqrm_lf']
PAPER_EVENT_LABELS = ['Restatement', 'Asset Impair.', 'Acct. Estimate',
                      'ICW Manager', 'ICW Auditor', 'Going Concern', 'Late Filing']

def results_to_df(res, n_obs):
    """Coefficient table with N and Adj. R² appended as footer rows."""
    df = pd.DataFrame({
        'coef':    res.params,
        'std_err': res.bse,
        't':       res.tvalues,
    ...
    footer = pd.DataFrame({
        'coef':    [int(n_obs), round(getattr(res, 'rsquared_adj', np.nan), 4)],
        'std_err': [np.nan, np.nan],
        't':       [np.nan, np.nan],
        'p':       [np.nan, np.nan],
    }, index=['N', 'Adj. R2'])
    return pd.concat([df, footer])

def results_to_df_lean(res, n_obs, fe_counts: dict):
    """Coefficient table without FE rows; N, Adj. R², and FE counts as footer."""
    df = pd.DataFrame({
        'coef':    res.params,
        'std_err': res.bse,
        't':       res.tvalues,
    ...
    # Drop fixed-effect rows
    fe_rows = [c for c in df.index if str(c).startswith(FE_PREFIXES)]
    df = df.drop(index=fe_rows)

    footer_labels = ['N', 'Adj. R2'] + list(fe_counts.keys())
    footer_values = [int(n_obs), round(getattr(res, 'rsquared_adj', np.nan), 4)] + list(fe_counts.values())
    footer = pd.DataFrame({
        'coef':    footer_values,
        'std_err': [np.nan] * len(footer_labels),
        't':       [np.nan] * len(footer_labels),
        'p':       [np.nan] * len(footer_labels),
    }, index=footer_labels)
    return pd.concat([df, footer])

def build_pivoted_summary(fitted, lean=False):
    """
    Pivot summary with variables as rows and (sample × event) as column pairs.
    Each event gets two columns: coef and t-stat, side by side.
    fitted: {(event_key, sample_key): statsmodels result or None}
    lean=True: keep only the 8 key AA variables.
    """
    sample_order = [('all', 'All Lenders'), ('lead', 'Lead Only')]

    # Get variable list from first available result, excluding FE dummies
    key_vars = []
    for samp_key, _ in sample_order:
        for ev in PAPER_EVENTS:
            res = fitted.get((ev, samp_key))
            if res is not None:
                key_vars = [v for v in res.params.index
                            if not str(v).startswith(FE_PREFIXES)]
                break
        if key_vars:
            break

    if lean:
        key_vars = [v for v in key_vars
                    if v.startswith('Ipos_') or v.startswith('AA_')]

    stat_rows = ['N', 'Adj. R2']
    rows = key_vars + stat_rows

    cols, data = [], {}
    for samp_key, samp_label in sample_order:
        for ev_key, ev_label in zip(PAPER_EVENTS, PAPER_EVENT_LABELS):
            coef_col = f'[{samp_label}] {ev_label} | coef'
            t_col    = f'[{samp_label}] {ev_label} | t'
            cols += [coef_col, t_col]
            res = fitted.get((ev_key, samp_key))
            if res is not None:
                data[coef_col] = {v: res.params.get(v, np.nan) for v in key_vars}
                data[t_col]    = {v: res.tvalues.get(v, np.nan) for v in key_vars}
                data[coef_col]['N']       = int(res.nobs)
                data[coef_col]['Adj. R2'] = round(res.rsquared_adj, 4)
                data[t_col]['N']          = np.nan
                data[t_col]['Adj. R2']    = np.nan
            else:
                data[coef_col] = {v: np.nan for v in rows}
                data[t_col]    = {v: np.nan for v in rows}

    return pd.DataFrame(data, index=rows)[cols]
...
def run_all(input_main, input_counts, output_xlsx):
    df = load_csv(input_main)
    counts = load_csv(input_counts)
    ...
    results_B, results_D = [], []
    fitted_B, fitted_D = {}, {}  # (event_key, sample_key) -> fitted result

    Path(output_xlsx).parent.mkdir(parents=True, exist_ok=True)
    output_lean = str(output_xlsx).replace('.xlsx', '_lean.xlsx')
    with pd.ExcelWriter(output_xlsx, engine='xlsxwriter') as xw, \
         pd.ExcelWriter(output_lean, engine='xlsxwriter') as xw_lean:
        for event_key, root in EVENTS.items():
            for sample_key, sconf in SAMPLES.items():
                ...
                # RHS: 8 AA variables only (no OtherClientN, no control variables)
                rhs = build_rhs_aa_columns_LOG_IND(df_use, event_key, sample_key, counts)
                rhs = rhs.drop(columns=['OtherClientN'], errors='ignore')
                ...
                try:
                    ...
                    print(f"[WARN] FE-B {event_key}/{sample_key} skipped: {_e}")
                    pd.DataFrame({'note': [str(_e)]}).to_excel(xw,      sheet_name=sheet_name_B)
                    pd.DataFrame({'note': [str(_e)]}).to_excel(xw_lean, sheet_name=sheet_name_B)
                    results_B.append({'structure':'B','event':event_key,'sample':sample_key,'error':str(_e)})
                    res_B = None
                if res_B is not None:
                    results_to_df(res_B, X_B.shape[0]).to_excel(xw, sheet_name=sheet_name_B)
                    ...
                    b_cols_B  = [c for c in X_B.columns if c.startswith('b_')]
                    clusters_borrower_B = int(pd.Series(clustersB).nunique())

                    fe_counts_B = {
                        'Industry×Year FEs': len(iy_cols_B),
                        'Borrower FEs':      len(b_cols_B),
                        'Lender FEs':        len(l_cols_B),
                    }
                    results_to_df_lean(res_B, X_B.shape[0], fe_counts_B).to_excel(xw_lean, sheet_name=sheet_name_B)
                    fitted_B[(event_key, sample_key)] = res_B
                    ...
                    })  # end results_B.append
                # end if res_B is not None
                ...
                try:
                    ...
                    print(f"[WARN] FE-D {event_key}/{sample_key} skipped: {_e}")
                    pd.DataFrame({'note': [str(_e)]}).to_excel(xw,      sheet_name=sheet_name_D)
                    pd.DataFrame({'note': [str(_e)]}).to_excel(xw_lean, sheet_name=sheet_name_D)
                    results_D.append({'structure':'D','event':event_key,'sample':sample_key,'error':str(_e)})
                    res_D = None
                if res_D is not None:
                    results_to_df(res_D, X_D.shape[0]).to_excel(xw, sheet_name=sheet_name_D)
                    ...
                    fe_counts_D = {
                        'Industry×Year FEs': len(iy_cols_D),
                        'Pair FEs':          len(p_cols_D),
                    }
                    results_to_df_lean(res_D, X_D.shape[0], fe_counts_D).to_excel(xw_lean, sheet_name=sheet_name_D)
                    fitted_D[(event_key, sample_key)] = res_D
                    ...
                    })  # end results_D.append
                # end if res_D is not None

        # Summary sheets
        build_pivoted_summary(fitted_B, lean=False).to_excel(xw,      sheet_name='Summary_FE_B')
        build_pivoted_summary(fitted_D, lean=False).to_excel(xw,      sheet_name='Summary_FE_D')
        build_pivoted_summary(fitted_B, lean=True).to_excel(xw_lean,  sheet_name='Summary_FE_B')
        build_pivoted_summary(fitted_D, lean=True).to_excel(xw_lean,  sheet_name='Summary_FE_D')


# ----------------------------- CLI -----------------------------
if __name__ == '__main__':
    p = argparse.ArgumentParser()
    p.add_argument('--main',   default=BOX_MAIN,
                   help='local path or Box URL for full_model_data.csv')
    p.add_argument('--counts', default=BOX_COUNTS,
                   help='local path or Box URL for parent_other_counts.csv')
    p.add_argument('--out',    default='results/regression_results_initial.xlsx',
