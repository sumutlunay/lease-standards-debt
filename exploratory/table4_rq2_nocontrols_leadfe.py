"""
table4_rq2_nocontrols.py  —  Table 4: RQ2 lender experience, no controls
=========================================================================

RQ2 lender-experience results in the NO-CONTROLS specification, in the manuscript's
Table 4 layout: one sheet, two stacked panels, seven event-family columns each.

  Window          : 12-month lookback only
  RHS             : ONLY the four experience test variables + the Model-7 fixed effects
                    (Industry×Year + Borrower + Lender multi-hot) — no determinants,
                    no controls
...
SAMPLE IS PINNED to the Model-7 sample: a row enters iff the experience variables AND the
determinants AND the controls are all non-missing — exactly as in table3 — so N is
row-comparable and only the RHS controls are removed, never the sample. The determinants
and controls appear ONLY in that mask; they are never regressors here.

★ RATING SPEC. The determinant list now carries the credit-quality BUCKETS
(BB_grade / B_grade / CCC_below / non_rated_suppl_all, with ig_grade the omitted reference)
in place of the linear num_rating_suppl_all, for consistency with the rest of the pipeline
(7985df5). Because the rating enters this script ONLY through the sample mask, and the
buckets are non-missing exactly where the linear rating is, this leaves N and every
coefficient unchanged — it is a consistency change, not a specification change.

Test variables are log(1+x) counts of irregularity events at the syndicate's lenders in
their OTHER borrowers, so they are not winsorized (and nothing else here needs it). Event
families and the test-variable construction are identical to exploratory/07_rq2_experience.py.

Rehash of exploratory/07b_rq2_table4.py, trimmed (console and docstring condensed). All
estimation logic is unchanged.

★ LEAD-ARRANGER FE VARIANT (2026-09-02). Identical to code/manuscript/table4_rq2_nocontrols.py in every
...
same row and column layout. A coefficient that moves between table4_leadfe.xlsx and table4.xlsx moved because of
...
Output: output/tables/table4_leadfe.xlsx  (single sheet 'Table 4', two panels)
"""
...
OUT_FILE = OUT_DIR / "table4_leadfe.xlsx"
SHEET    = "Table 4"
...
# ★ Used ONLY to pin the sample (never regressors here). Bucket ratings; ig_grade omitted.
...
def run_experience_model(df, dv, X_fe, fe_bor, fe_lender, event, sample_suf) -> list:
    ...
    # Sample pinned to Model 7: determinants/controls only gate rows, never enter the model.
    finite  = np.isfinite(df[experience + DETERMINANTS + CONTROLS].to_numpy(dtype=float)).all(axis=1)
    ...
    X, y, cl = stabilize_design(X_full, df.loc[sub, dv], df.loc[sub, "gvkey"])
